-- Model: fitnets_firsthalf8.def.lua
...
function createModel()
   ...
   local depths = torch.Tensor{8, 10, 12, 14, 16, 18, 20, 22, 24}

   -- Layer 1: Convolution with filter size 3, step size 2, padding 1
   -- Output size: 48 x 48 x depths[1] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(3, depths[1], 3, 3, 2, 2, 1, 1))
   net:add(nn.SpatialBatchNormalization(depths[1]))
   net:add(nn.ReLU())

   -- Layer 4: Convolution with filter size 3, step size 2, padding 1
   -- Output size: 24 x 24 x depths[2] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(depths[1], depths[2], 3, 3, 2, 2, 1, 1))
   net:add(nn.SpatialBatchNormalization(depths[2]))
   net:add(nn.ReLU())

   -- Layer 7: Convolution with filter size 3, step size 1, padding 1
   -- Output size: 24 x 24 x depths[3] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(depths[2], depths[3], 3, 3, 1, 1, 1, 1))
   net:add(nn.SpatialBatchNormalization(depths[3]))
   net:add(nn.ReLU())

   -- Layer 10: Convolution with filter size 3, step size 1, padding 1
   -- Output size: 12 x 12 x depths[4] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(depths[3], depths[4], 3, 3, 2, 2, 1, 1))
   net:add(nn.SpatialBatchNormalization(depths[4]))
   net:add(nn.ReLU())

   -- Layer 13: Convolution with filter size 3, step size 2, padding 1
   -- Output size: 6 x 6 x depths[5] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(depths[4], depths[5], 3, 3, 2, 2, 1, 1))
   ...
   -- net:add(nn.Reshape(162))

   -- -- 6 * 6 * 640 = 23040
   -- -- 3 * 3 * 18 = 162
   -- net:add(nn.Linear(162, 23040))
