...
function createModel()
   ...
   -- The depths are the same as in fitnets_all5.def.lua and fitnets_all6.def.lua
   -- The only difference is the depths.
   local depths = torch.Tensor{8, 10, 12, 16, 20, 24, 28, 32, 36}

   -- Layer 1: Convolution with filter size 5, step size 2, padding 2
   -- Output size: 48 x 48 x depths[1] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(3, depths[1], 5, 5, 2, 2, 2, 2))
   net:add(nn.SpatialBatchNormalization(depths[1]))
   net:add(nn.ReLU())

   -- Layer 4: Convolution with filter size 5, step size 2, padding 2
   -- Output size: 24 x 24 x depths[2] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(depths[1], depths[2], 5, 5, 2, 2, 2, 2))
