...
function createModel()
   ...
   -- Layer 1: Convolution with filter size 7, step size 1, padding 3
   -- Output size: 96 x 96 x depths[1] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(3, depths[1], 7, 7, 1, 1, 3, 3))
   net:add(nn.SpatialBatchNormalization(depths[1]))
   net:add(nn.ReLU())

   -- Layer 4: Convolution with filter size 7, step size 2, padding 3
   -- Output size: 48 x 48 x depths[2] (width, height, depth)
   ...
   -- Output size: 48 x 48 x depths[3] (width, height, depth)
   ...
   -- Output size: 3 x 3 x depths[10] (width, height, depth)
   net:add(nn.SpatialConvolutionMM(depths[9], depths[10], 3, 3, 1, 1, 1, 1))
   net:add(nn.SpatialBatchNormalization(depths[10]))
   net:add(nn.ReLU())
