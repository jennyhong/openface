...
do
  if $USE_GPU ; then
    qlua main.lua -data $DATA_DIR -modelDef $MODEL_DIR/fitnets_firsthalf$i.def.lua -trainFirstHalf -cache $WORK_DIR-$i  -nEpochs $NEPOCHS;
    qlua main.lua -data $DATA_DIR -modelDef $MODEL_DIR/fitnets_all$i.def.lua -trainFullStudent -cache $FINAL_WORK_DIR-$i  -firstHalfCached $WORK_DIR-$i/1/model_$NEPOCHS.t7;
  else
